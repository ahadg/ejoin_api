  ...
  try {
    ...
    if (status && !['online', 'offline', 'warning'].includes(status)) {
      return res.status(400).json({
        code: 400,
        reason: 'Invalid status. Must be online, offline, or warning'
      });
    }
    ...
    res.status(500).json({
      code: 500,
      reason: 'Error updating device status'
    });
  }
});
...
router.patch('/:id/stats', auth, async (req, res) => {
  try {
    ...
    res.json({
      code: 200,
      message: 'Device stats updated successfully',
      data: { device }
    });
  } catch (error) {
    ...
    res.status(500).json({
      code: 500,
      reason: 'Error updating device stats'
    });
  }
});

// Reset daily sent count (typically called by a cron job)
router.post('/:id/reset-daily-count', auth, async (req, res) => {
  try {
    const device = await deviceModel.findOneAndUpdate(
      { _id: req.params.id, user: req.user._id },
    ...
    res.status(500).json({
      code: 500,
      reason: 'Error resetting daily count'
    });
  }
});
